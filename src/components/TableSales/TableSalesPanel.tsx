...
const CashRegisterCloseConfirmation: React.FC<CashRegisterCloseConfirmationProps> = ({
  ...
  const loadTableSale = async (table: RestaurantTable) => {
    if (!table.current_sale_id) {
      // Mesa sem venda, limpar carrinho
      setCart([]);
      setCustomerName('');
      setCustomerCount(1);
  ...
  const addToCart = (product: any) => {
    // Se é produto pesável, abrir modal para inserir peso
  ...
  const addProductToCart = (product: any, quantity: number = 1, weight?: number) => {
    ...
    if (existingItem) {
      ...
      if (product.is_weighable && newWeight) {
        newSubtotal = newWeight * (product.price_per_gram || 0) * 1000; // peso em kg * preço por grama * 1000
      } else {
